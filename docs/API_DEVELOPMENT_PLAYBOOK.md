...
- **Critical dependency contract** – `ConfigService`, Redis clients, and cache services are required dependencies for modules that use them. Do not mark them optional with `@Optional()` and do not add `if (!this.redis)` style bypasses. Misconfiguration must fail fast at startup.
...
- All exceptions must include errorCode imported from @/common/types/error-codes — never inline strings.
- Rate limiting in controllers must use RateLimitExceededException from @/common/exceptions.
...
### 4.1 Cross-System ID Policy

Use UUIDv7 for cross-system string identifiers such as:

- `requestId`
- `correlationId`
- async workflow IDs
- tracing/logging IDs that travel between systems
...
Keep relational DB keys unchanged:

- table primary keys and foreign keys remain `serial/int` where defined by the schema
- auth tables keep their existing ID strategy unless an explicit auth migration is planned

### 4.2 Shared Outbox Queue Ownership Contract

When using shared `outbox_events`, routing ownership must be explicit and enforced in claim queries.

- Every outbox producer must set `targetQueue` explicitly.
- Any claimer that reads `pending` rows **must strictly filter to its owned `targetQueue` values**.  
- Claimers must **not** attempt cross-queue handling by default.
- Never rely on status-only claiming (`status = "pending"`) without queue ownership filtering.
- If a new module later writes `targetQueue=<new_queue>` into `outbox_events`, it must either:
  - have its own dispatcher/claimer, or
  - be explicitly added to an existing dispatcher’s owned queue set and tests.
...
- **Auth exception** – Do not change existing auth ID strategy in routine feature work; auth IDs remain as-is unless an explicit auth migration effort is planned.
...
- **Throw typed exceptions with sturctured error objects** –- never plain text messages alone:
  throw new ConflictException({ message: "Promotion code already exists.", errorCode: "PROMOTION_CODE_EXISTS" });
- All error codes must be imported from apps/api/src/common/types/error-codes.ts
- Use RateLimitExceededException from @/common/exceptions for all rate limiting — never HttpException with TOO_MANY_REQUESTS directly
...
7. [ ]  Errors use structured { message, errorCode } format — never plain text. ErrorCodes imported from common/types/error-codes.ts.
