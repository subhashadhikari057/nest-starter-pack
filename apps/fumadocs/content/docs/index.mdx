...
description: A modern full-stack TypeScript monorepo combining React 19, NestJS, TanStack Router, Drizzle ORM, and PostgreSQL - all managed with Turborepo and pnpm workspaces.
---
...
### Packages
- **@bullhouse/db** - Centralized database schemas and migrations
- **@bullhouse/email** - Email service with SMTP support
- **@bullhouse/otp** - OTP generation and validation
- **@bullhouse/storage** - File storage (local, S3, Supabase)
...
> **Note:** If you prefer to use a local PostgreSQL installation instead of Docker, you can skip Docker and install PostgreSQL manually from [postgresql.org](https://www.postgresql.org/download/)
...
- **`@bullhouse/db`**: Centralized database layer using Drizzle ORM. Contains schemas, migrations, and database connection. Includes Docker Compose for easy PostgreSQL setup.
- **`@bullhouse/email`**: Reusable email service for sending transactional emails via SMTP.
- **`@bullhouse/otp`**: OTP generation and validation utilities for two-factor authentication.
- **`@bullhouse/storage`**: File upload and storage management supporting local filesystem and cloud storage providers.
...
- **API Documentation**: [http://localhost:5000/api](http://localhost:5000/api) (Swagger/OpenAPI docs)
- **Database Studio**: Run `pnpm run db:studio` and visit [https://local.drizzle.studio](https://local.drizzle.studio)
...
Before `apps/api` starts, workspace package dev tasks (`@bullhouse/db`, `@bullhouse/jobs`, `@bullhouse/redis`, `@bullhouse/realtime-core`, `@bullhouse/mongodb`, `@bullhouse/firebase`) are orchestrated first so linked `dist` outputs are ready.
...
pnpm add <package-name> --filter @bullhouse/db      # Database package
pnpm add <package-name> --filter @bullhouse/email   # Email package
pnpm add <package-name> --filter @bullhouse/otp     # OTP package
pnpm add <package-name> --filter @bullhouse/storage # Storage package
```
...
**Checklist:**
1. Verify backend is running at [http://localhost:5000](http://localhost:5000)
...
4. Review the [Drizzle ORM documentation](https://orm.drizzle.team/) for database issues
